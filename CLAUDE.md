...
### LLM 降级模式
每个节点（planner、analyst、fact_checker）都遵循相同模式：
1. 若 `settings.llm_api_key` 非空 → 调用 LLM（通过 LiteLLM）
2. LLM 失败或 key 为空 → 降级为规则逻辑（仍可运行，无报错）
...
### 搜索服务
`SearchService`（`app/services/search_service.py`）通过 `SEARCH_PROVIDER` 配置选择引擎（`duckduckgo` 或 `tavily`），`multi_search` 并发执行多关键词搜索并按 URL 去重。DuckDuckGo 走 `asyncio.to_thread` 包装同步接口。
...
- `LLM_API_KEY` 为空时系统自动降级为规则模式
- `SEARCH_PROVIDER=duckduckgo` 无需 API key（适合本地开发）
- Qdrant 默认内存模式，无需 Docker

## API 端点
- `POST /api/research` — 创建任务（异步后台执行，参数见 `ResearchTaskCreate`）
